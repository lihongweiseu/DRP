...
for i in range(1,21):
    ...
    torch.save(best_params.state_dict(), './saved_models/'+name+'_LSTM_'+str(hidden_s)+'_'+'{:.4e}'.format(loss_m).replace('e+0', 'e').replace('e-0', 'e-')+'_'+str(im)+'_'+cost_time_str+'.pt')
...
# 0.9985540858598083, 0.9529907025104606, 56m58s, 0.08827733993530273s, 1539
...
# %%
temp=0.025123456789
savemat('./MR_LSTM_'+'{:.5e}'.format(temp).replace('e+0', 'e').replace('e-0', 'e-')+'_.mat', {'temp': temp})
# %%
